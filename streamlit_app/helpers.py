...
def _get_profiles() -> list[str]:
    ...
    try:
        ...
        return ["architecture", "bcdr", "security", "cost", "governance", "networking"]
...
def render_sidebar() -> Path | None:
    ...
    st.caption("⚠️ **POC / Experimental** — This tool is a proof-of-concept "
               "and is not intended for production use.")
    runs = list_runs()

    st.sidebar.header("📂 Assessment Run")

    if not runs:
        st.sidebar.info("No runs found yet.")
        st.sidebar.divider()
...
def _render_sidebar_footer() -> None:
    """Contact card and POC disclaimer at the bottom of the sidebar."""
    st.sidebar.divider()
    st.sidebar.markdown(
        "**Diego Martos**  \n"
    ...
    st.sidebar.caption(
        "⚠️ **POC / Experimental** — This tool is a proof-of-concept "
        "and is not intended for production use."
    )
