...
sequenceDiagram
    ...
    Note over CLI: User selects deep-dive profile<br/>(architecture | bcdr | security | cost<br/>| governance | networking | defender)
...
modules/
  ...
  kql/                              # Modular KQL query library
    ...
    cost.py                         # Supplementary queries for cost optimization
...
The script authenticates via `az login --tenant <tenant-id>`. This opens an interactive browser-based login flow (or reuses cached credentials if already signed in). Service principal and managed identity authentication can be configured via standard Azure CLI methods (`az login --service-principal`).
...
# --- Token limits (all optional — defaults shown) ---
# BRIEF  = Phase 1 Environment Brief.
# DOC    = Phase 2 deep-dive report narrative.
# MERMAID = Phase 2 Mermaid diagram.
...
# Optional. Default is 500. Azure enforces a max of 1000.
RESOURCE_GRAPH_PAGE_SIZE=500

# --- Prompt profile (default when pressing Enter at the menu) ---
# Valid values: architecture, bcdr, security, cost, governance, networking, defender
...
```bash
# Interactive mode — prompted for profile selection after Phase 1
python AI_Inventory_Architect.py

# Non-interactive mode — skip the menu and run a specific profile directly
python AI_Inventory_Architect.py --profile security
```

The `--profile` flag accepts any valid profile ID (`architecture`, `bcdr`, `security`, `cost`, `governance`, `networking`, `defender`). When provided, Phase 1 discovery still runs but the interactive menu is skipped.
...
| `401 Unauthorized` from Azure OpenAI | Invalid API key or mismatched endpoint | Verify `AZURE_OPENAI_ENDPOINT` and `AZURE_OPENAI_API_KEY` in `.env`. Endpoint format: `https://<resource>.openai.azure.com` |
...
modules/kql/
  ...
  cost.py              # + Advisor cost recommendations, orphaned disks, App Service plan details
