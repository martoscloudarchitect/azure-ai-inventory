...
def load(project_dir: Path) -> dict:
    ...
    if endpoint and api_key:
        ...
        cfg["doc_max_tokens"] = _get_optional_int("AZURE_OPENAI_DOC_MAX_COMPLETION_TOKENS")
        cfg["mermaid_max_tokens"] = _get_optional_int("AZURE_OPENAI_MERMAID_MAX_COMPLETION_TOKENS")

        if not cfg["doc_max_tokens"] or not cfg["mermaid_max_tokens"]:
            raise RuntimeError(
                "Set AZURE_OPENAI_DOC_MAX_COMPLETION_TOKENS and "
                "AZURE_OPENAI_MERMAID_MAX_COMPLETION_TOKENS in .env."
            )
