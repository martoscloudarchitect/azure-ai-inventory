...
class TokenTracker:
    ...
    def report(self, label: str, usage: dict, max_tokens: int) -> None:
        ...
        estimated_input = usage.get("estimated_input_tokens", prompt)
        max_input_tokens = usage.get("max_input_tokens", 0)
        input_usage_percent = usage.get("input_usage_percent", 0)
        pct = round((completion / max_tokens) * 100, 2) if max_tokens else 0
        remaining = max(0, max_tokens - completion)

        print(
            f"{label} tokens -> est input: {estimated_input}, prompt: {prompt}, "
